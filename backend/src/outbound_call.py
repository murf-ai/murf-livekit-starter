...
async def main():
    ...
    if not url or not api_key or not api_secret:
        logger.error(
            "LiveKit credentials not found in environment variables. Please check backend/.env.local"
        )
        sys.exit(1)
    ...
    sip_trunk_id = os.getenv("LIVEKIT_SIP_TRUNK_ID", "ST_UkPNPeC7WUrw")
    ...
    try:
        ...
        logger.warning(
            f"Could not create agent dispatch (it may already be running or configured automatically): {e}"
        )
